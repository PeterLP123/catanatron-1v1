#!/usr/bin/env python3
"""Operational and information-boundary gate for public-chance PUCT."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--minimum-search-decisions", type=int, default=1200)
    parser.add_argument("--minimum-multi-ply-decisions", type=int, default=600)
    parser.add_argument("--minimum-chance-actions", type=int, default=100)
    parser.add_argument("--minimum-chance-outcomes", type=int, default=300)
...
def main(argv: list[str] | None = None) -> int:
    ...
    positive_counts = (
        ...
        args.minimum_chance_actions,
        args.minimum_chance_outcomes,
    )
    if any(value < 1 for value in positive_counts):
        raise ValueError("game and minimum-count gates must be positive")
    ...
    if not manifest.is_file():
        raise FileNotFoundError(f"Missing visible chance PUCT manifest: {manifest}")
    ...
    scalar_keys = (
        ...
        "chance_actions_expanded",
        "chance_outcomes_expanded",
        "chance_root_decisions",
    ...
    for game_index in range(args.games):
        game_seed = derive_seed(
            "visible-chance-puct-diagnostic-game",
            base_seed=args.seed,
            iteration=0,
        ...
        student = VisibleChancePuctPlayer(student_color, manifest)
    ...
    gates = [
        ...
        (
            "minimum_chance_actions",
            totals["chance_actions_expanded"] >= args.minimum_chance_actions,
            totals["chance_actions_expanded"],
            args.minimum_chance_actions,
        ),
        (
            "minimum_chance_outcomes",
            totals["chance_outcomes_expanded"] >= args.minimum_chance_outcomes,
            totals["chance_outcomes_expanded"],
            args.minimum_chance_outcomes,
        ),
        (
            "minimum_change_rate",
        ...
        (
            "zero_probability_sum_violations",
            totals["probability_sum_violations"] == 0,
            totals["probability_sum_violations"],
            0,
        ),
        (
    ...
    report = {
        "schema_version": "1.0",
        "kind": "visible_public_chance_puct_operational_diagnostic",
    ...
    print(
        f"visible chance PUCT diagnostic: searches={searches} "
        f"multi_ply={totals['multi_ply_decisions']} "
        f"chance_actions={totals['chance_actions_expanded']} "
